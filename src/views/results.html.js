...
const createListItems = (results) => {
  ...
  for (const item of results) {
    ...
    <li><p class='selected'>selected: ${
      item.answers[localStorage[results.indexOf(item)]]
    }</p></li>
    <p><span class='correct'>Learn about it :</span> ${item.description} <p>
    <p>${item.links[0].text} : <a href='${item.links[0].href}'>${
        item.links[0].href
      }</a></p>
    <p>${item.links[1].text} : <a href='${item.links[1].href}'>${
        item.links[1].href
      }</a></p>
        </div>
        </div>
...
<h5 class='mb-5'>Correct answers : ${correctAnswerResult} / ${
    results.length
  }</h5>
<h5 class='mb-5'>Correct Average : ${
